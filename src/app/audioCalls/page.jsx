...
const AudioCalls = () => {
    ...
    const allCallLogs = timelineData.filter(item => item.action === "Call");
    const sortedCallLogs = allCallLogs.sort((a, b) => Date.parse(b.fullTime) - Date.parse(a.fullTime));
    // console.log(timelineData);
    return (
        <div className=" max-w-7xl h-full text-black">
            <div className="flex p-10">
                <h1 className="text-2xl font-bold">All Audio Call List </h1>
                <Link href="/timeline"><button className="btn btn-primary px-10 mx-10">Back <ArrowRight /></button></Link>
            </div>
            <div className=" p-5 space-y-3">

                {/* icon */}
                {sortedCallLogs.map(data =>
